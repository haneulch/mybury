...
public class HostController {
   ...
   */
  @Operation(summary = "홈",
    requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "버킷리스트 목록 request",
  ...
  private List<MyPageResponseVO.CategoryVO> setCategoryList(User user) {
    ...
    userCategoryList =
      userCategoryList.stream().sorted(Comparator.comparing(Category::getPriority)).collect(Collectors.toList());
